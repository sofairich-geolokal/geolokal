...
interface UserStatsRow {
  ...
  superadmin_count: string | number;
  admin_count: string | number;
  lgu_count: string | number;
  viewer_count: string | number;
  new_users_30_days: string | number;
  active_users_7_days: string | number;
  active_users: string | number;
}

interface LguStatsRow {
  total_lgus: string | number;
  active_lgus: string | number;
  viewer_lgus: string | number;
  users_with_lgu: string | number;
}
...
interface ProjectStatsRow {
  ...
  new_projects_30_days: string | number;
  updated_projects_7_days: string | number;
}

interface MapStatsRow {
  total_maps: string | number;
  new_maps_30_days: string | number;
  unique_categories: string | number;
  visible_maps: string | number;
  downloadable_maps: string | number;
  maps_with_owner: string | number;
}

interface AuditStatsRow {
  total_activities: string | number;
  activities_7_days: string | number;
  activities_today: string | number;
  activities_1_hour: string | number;
  unique_actors: string | number;
  unique_actions: string | number;
}

interface ExportStatsRow {
  total_exports: string | number;
  exports_30_days: string | number;
  completed_exports: string | number;
  pending_exports: string | number;
  active_exports: string | number;
}

interface StorageStatsRow {
  total_geometry_size: string | number;
  total_metadata_size: string | number;
  layers_with_geometry: string | number;
  layers_with_metadata: string | number;
}

interface SystemHealthRow {
  active_contributors: string | number;
  daily_new_layers: string | number;
  weekly_updated_layers: string | number;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const authResult = (await query('SELECT role FROM users WHERE id = $1', [userId])) as { rows: UserRoleRow[] };
    ...
    `)) as { rows: SystemHealthRow[] };

    const responseData = {
      users: userStats.rows[0] || {},
      lgus: lguStats.rows[0] || {},
      projects: projectStats.rows[0] || {},
      maps: mapStats.rows[0] || {},
      activities: auditStats.rows[0] || {},
    ...
    return NextResponse.json(
      { error: 'Failed to fetch admin dashboard statistics', details: errorMessage },
      { status: 500 }
    );
  }
}